...
class PM:
    ...
    def Learning_FIsnetwork_GANs(self, edge_list, data_for_GANs,n):
        ...
        def prepare(adjacency_matrix, n_input, n_hidden, n_noise, stddev):
            ...
            # Generator weights
            gw1 = tf.Variable(tf.random_normal([n_noise, int(n_genes/10)], stddev=0.01))
            gw2 = tf.Variable(tf.random_normal([int(n_genes/10), n_genes], stddev=0.01))
            ...
            D_W3 = tf.Variable(tf.random_normal([int(n_hidden/2), int(n_hidden/4)], stddev=0.01))
            ...
            return reconstucted_network_adjacency_matrix, X, Z, gw1,gw2,gw3,gw4,gw5, D_W1, D_W2,D_W3,D_W4,D_W5

        # generator of GANs.
        def generator(gw1, gw2,gw3,gw4,gw5, reconstucted_network_adjacency_matrix, noise_z):
            hidden1 = tf.nn.relu(tf.matmul(noise_z, gw1))
            hidden2 = tf.nn.relu(tf.matmul(hidden1, gw2))
            hidden3 = tf.nn.relu(tf.matmul(hidden2,reconstucted_network_adjacency_matrix * (gw3 * tf.transpose(gw3))))
            hidden4 = tf.nn.relu(tf.matmul(hidden3,reconstucted_network_adjacency_matrix * (gw4 * tf.transpose(gw4))))
            output = tf.nn.relu(tf.matmul(hidden4,gw5))
            return output


        # discriminator of GANs.
        def discriminator(inputs, D_W1, D_W2,D_W3,D_W4,D_W5):
        ...
        reconstucted_network_adjacency_matrix, X, Z, gw1,gw2,gw3,gw4,gw5, D_W1, D_W2,D_W3,D_W4,D_W5 = prepare(adjacency_matrix, n_genes, 512, n_genes,
                                                                               0.01)

        G = generator(gw1, gw2,gw3,gw4,gw5, reconstucted_network_adjacency_matrix, Z)

        D_gene = discriminator(G, D_W1, D_W2,D_W3,D_W4,D_W5)
        # D_gene = D_gene.assign( tf.where (tf.equal(D_gene, tf.constant(0)), tf.constant(epsilon), D_gene) )
        D_real = discriminator(X, D_W1, D_W2,D_W3,D_W4,D_W5)
        ...
        G_var_list = [gw1,gw2,gw3,gw4,gw5]
        ...
        gradients = tf.gradients(discriminator(interpolates,D_W1,D_W2,D_W3,D_W4,D_W5), [interpolates])[0]
