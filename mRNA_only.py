...
def main():
	...
	for foldnum in range(10):
		...
		for i in range(pm.n_experiment):
			result_GANs.append(pm.Learning_FIsnetwork_GANs(i, reconstructed_FIs_perfold[foldnum],data_for_GANs,foldnum))
...
def loading_data():		
	...
	# CNA=read_file("BRCA_CNA.txt")
	# met=read_file("BRCA_methylation.txt")
	# snp=read_file("BRCA_SNP.txt")
...
def preprocessing():
	...
	raw_mRNA2,edge_list,lable,gene_names,gene2num,num2gene,small_gene2num=intersetion_data(raw_mRNA,raw_edges,raw_lable,genes,num2gene,gene2num)
	...
	Pm=PM(n_gene_in_ttest,n_biomarker,damping_factor, n_experiment,n_limit,mRNA,lable,edge_list,good_train_samples,bad_train_samples,test_samples,gene2num,num2gene,mRNA_ttest,sample2id,small_gene2num)
...
class PM:
	...
	"""
	def __init__(self,n_gene_in_ttest,n_biomarker,damping_factor, n_experiment,n_limit,mRNA,lable,edge_list,good_train_samples,bad_train_samples,test_samples,gene2num,num2gene,mRNA_ttest,sample2id,small_gene2num):
	...
	def make_adjacencyMatrix(self,edge_list):
		...
		for edge in edge_list:
			...
			matrix[y][x] =1
		return matrix
	def get_data_gene(self,bi,sample):
		#print(sample)
		#print(d)
		count = 0
		temp = []
		for samp in sample:
			id = self.sample2id[samp]
			temp.append(self.mRNA[bi][id])
			count+=1
		print(count)
	...
	def auc(self,reconstructed_FIs,biomarkerperfold) :
		...
		for foldnum in range(10):
			...
			#sort genes by t-statistics
			mRNA_t_full = []
			for fold in self.mRNA_ttest:
				for item in fold:
					mRNA_t_full.append(item)
			mRNA_t_full.sort(key=lambda tup: tup[0],reverse=True)
			...
			for bi in biomarker:
				...
				if bi in mRNA_t:
					if bi in mRNA_t:
						data_tmp.append(self.get_data_gene(bi,sample))
						# data_tmp.append(self.mRNA.loc[bi,sample].values.astype('float64'))
						test_tmp.append(self.get_data_gene(bi,test_sample))
					# if bi in CNA_t:
			...
			#print(self.lable)
			# print(self.lable.type())
			# print(lable)
			# print(test_sample)
			testlable = []
			trainlable = []
			for id in test_sample:
				testlable.append(int(self.lable[self.sample2id[id]]))
			for id in sample:
				trainlable.append(int(self.lable[self.sample2id[id]]))
			trainlable = np.asarray(trainlable)
